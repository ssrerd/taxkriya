<template>
  <div>
       <div class="thankyou-area">
          <div class="container">
             <div class="row thank">
                <i style="text-align:center;font-size:50px;color:white;filter: drop-shadow(2px 4px 6px rgba(0,0,0,0.3));" class="fas fa-check-circle"></i> 
                <div class="col">
                  <h1>THANK YOU!</h1>
                  <p style="text-align: center;">We have received your request. We value your business as much as you do.</p>
                 
      

                    <div class="row">
                      <h5 style="text-align:center;">Our experts will get in touch with you shortly.</h5>
                    </div>  
                   
                    <div class="row mt-3" style="width:10%;margin:0 auto auto auto;">  
                      <a href="/" style="width: auto;margin: auto;" class="btn btn-outline-primary">Home</a>
                    </div>
                </div>
     
  <!--========================================== End Search Banner-Area ==============================================-->                

              </div>
            </div>  
          </div>

        
        <Howitworks></Howitworks> 

  </div>    
</template>

<script>
export default {

}
</script>

<style>
.thankyou-area{
    background-color: var(--contrast-color);
    background-position: left center;
    background-size: 40%;
}


.thankyou-area .container{
    padding: 5% 2% 5% 2%;
}
.thankyou-area .row h1{
    text-align: center;
    font-size: 5em;
    filter: drop-shadow(2px 4px 6px rgba(0,0,0,0.3));
    color: white;
    
}
</style>